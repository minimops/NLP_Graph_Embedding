...
## Random Walk in node2vec
  ...
  The random walk is of 2\textsuperscript{nd} order, meaning that the previous node visited has an impact on the sampling of the next node of a RW traverse. In this case, the probability of a node $x$ to become the next node in the RW {$..., a, b$}, is biased not only by the weight of the edge ($b$, $x$) ($w_{i,j} = 1; \forall{i, j \in V}$ if the graph is unweighted), but also by the distance to the previous node $d_{a, x}$, where
  
$$
\alpha_{p,q}(a,x) = 
\begin{cases}
    \frac{1}{p},& \text{if } d_{a,x} = 0\\
    1,         &  \text{if } d_{a,x} = 1\\
    \frac{1}{q},& \text{if } d_{a,x} = 2
\end{cases}
$$
